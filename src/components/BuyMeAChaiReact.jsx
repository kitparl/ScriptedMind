...
const BuyMeAChai = () => {
	...
	const generateUpiUrl = (amount) => {
		const upiId = 'bisht.pranshu@ibl'
		const name = encodeURIComponent('Pranshu Singh Bisht')
		const note = encodeURIComponent(`Support - ₹${amount}`)
	...
	const handlePay = async () => {
		...
		try {
			...
			// Use qrToString to generate SVG
			const qrCodeSvg = await new Promise((resolve, reject) => {
				qrToString(upiUrl, { type: 'svg' }, (err, svg) => {
					if (err) reject(err)
					else resolve(svg)
				})
			})
			...
			setQr(qrCodeSvg)
			setR('QR Code generated. You can now scan it to make the payment.')
		} catch (error) {
			console.error('Error generating QR code:', error)
			setR('Error generating QR code. Please try again.')
		} finally {
	...
	return (
		<div className='upi-button-container'>
			...
			<p>Scan the QR code or use the UPI ID below to make a payment:</p>
			<p>
				<strong>bisht.pranshu@ibl</strong>
			</p>
			{r && (
				<p>
					<strong>{r}</strong>
