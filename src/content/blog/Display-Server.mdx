...
X11 and Wayland are most commonly associated with Linux, their usage is not strictly limited to it; they can be found in other Unix-like operating systems as well. However Windows uses the Windows Display Driver Model (WDDM) for graphics management and DirectX for high-performance graphics tasks. The Windows Composition Engine handles window compositing and rendering.
